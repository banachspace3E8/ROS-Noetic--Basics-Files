...
void callback_recieve_number_data(const std_msgs::Int64& msg){
	counter += msg.data;
...
bool callback_reset_counter(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res){
	...
	counter = 0;
	res.success = true;
	res.message = "Counter as been reset successfully";
	}
	else{
	res.success = false;
...
int main(int argc, char **argv){
	...
	ros::Subscriber sub = nh.subscribe("/number",1000, callback_recieve_number_data);
