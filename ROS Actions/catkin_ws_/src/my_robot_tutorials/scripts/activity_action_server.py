...
class CountUntilServer:
    ...
    def process_goal(self, goal_handle):
        ...
        while not rospy.is_shutdown():
            ...
            #    preempted = True
            #   break
            if self._cancel_goals[goal_handle.get_goal_id().id]:
                preempted = True
                break
    ...
    def on_goal(self,goal_handle):
        ...
        self._cancel_goals[goal_handle.get_goal_id().id] = False
       ...
       #change goal policy-> only one goal active
        ...
        w.start()
        #the goal could take some time to process everything but we need to also make sure if there is a cancel request, it is executed before the goal is complete, hence threading
