...
//C++ ROS Simple Action Server with ROS Noetic
#include <ros/ros.h>
...
class CountUntilServer{
	...
	public:
	//boost bind is used because we are working on a class
	CountUntilServer():_as(_nh,"/count_until",boost::bind(&CountUntilServer::onGoal, this, _1),false),_counter(0){
	
		_as.start();
		ROS_INFO("Simple Action Server has been started.");
	}

	void onGoal(const my_robot_msgs::CountUntilGoalConstPtr &goal){
		
		ROS_INFO("Goal Recieved");
		int max_number = goal->max_number;
		double wait_duration = goal->wait_duration;
		ROS_INFO("Max number: %d,wait duration : %1f",max_number, wait_duration);
