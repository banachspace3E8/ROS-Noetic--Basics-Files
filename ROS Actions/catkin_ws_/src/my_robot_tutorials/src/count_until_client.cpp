...
//C++ ROS Simple Action Client with ROS Noetic
#include <ros/ros.h>
...
class CountUntilClient{
	...
	ROS_INFO("Waitfor the action server to start...");
	_ac.waitForServer();
	...
	void sendGoal(){
		...
		_ac.sendGoal(goal,boost::bind(&CountUntilClient::doneCb,this,_1,_2),
				boost::bind(&CountUntilClient::activeCb,this),
				boost::bind(&CountUntilClient::feedbackCb,this,_1));
		ROS_INFO("Goal has been sent.");

		//Uncomment to cancel goal after 2 secs
		//ros::Duration(2.0).sleep();
		//_ac.cancelGoal();
	}

	void doneCb(const actionlib::SimpleClientGoalState& state,const my_robot_msgs::CountUntilResultConstPtr &result){
	...
	void feedbackCb(const my_robot_msgs::CountUntilFeedbackConstPtr &feedback){
		...
		ROS_INFO("Feedbackrecieved. Percentage: %1f", feedback->percentage);
	}
};
