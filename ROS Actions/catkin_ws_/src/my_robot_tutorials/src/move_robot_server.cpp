...
//C++ ROS Simple Action Server to move a robot with ROS Noetic
...
class MoveRobotServer{
	...
	public:
	MoveRobotServer():
		_as(_nh,"/move_robot",boost::bind(&MoveRobotServer::onGoal, this, _1),false),_current_position(50){
	...
	void onGoal(const my_robot_msgs::MoveRobotGoalConstPtr &goal){
		...
		ROS_INFO("Goal recieved");
                ...
                ROS_INFO("Goal position: %d, Velocity: %d", goal_position, velocity);
		bool success = false;
		...
		while (ros::ok() && !success && !invalid_parameters){
			if(_as.isPreemptRequested()){
				if(goal_position == _current_position){
					...
					message = "Prempted but already at goalposition";
					success = true;
				}
				else {
					message = "Preempted but stopped execuion";
					preempted = true;
				}
				break;
			}
