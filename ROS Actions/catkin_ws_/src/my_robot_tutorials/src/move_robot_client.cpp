...
//C++ ROS Simple Action Client to move a robot with ROS Noetic
...
class MoveRobotClient {
	...
	public:
	MoveRobotClient(): _ac("/move_robot",true){
	
	ROS_INFO("Wait for the action server to start...");
	_ac.waitForServer();
	ROS_INFO("Server is up now");

	_cancel_subscriber = _nh.subscribe("/cancel_move",10,&MoveRobotClient::cancelMoveCallback, this);
	}

	void cancelMoveCallback(const std_msgs::Empty &msg){
	
		ROS_INFO("Recieved message to cancel goal");
		_ac.cancelGoal();
	}
	...
	void sendGoal(){
		...
		goal.position = 99;
		goal.velocity = 4;
		_ac.sendGoal(goal,boost::bind(&MoveRobotClient::doneCb, this, _1,_2),boost::bind(&MoveRobotClient::activeCb, this),boost::bind(&MoveRobotClient::feedbackCb, this , _1));
		ROS_INFO("Goal has been sent");
	}

	void doneCb(const actionlib::SimpleClientGoalState& state, const my_robot_msgs::MoveRobotResultConstPtr &result){
	...
	void feedbackCb(const my_robot_msgs::MoveRobotFeedbackConstPtr &feedback){
		ROS_INFO("Feedback recieved. Current position: %d",(int)feedback->current_position);
	}
};
